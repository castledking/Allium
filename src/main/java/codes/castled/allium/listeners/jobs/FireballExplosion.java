package codes.castled.allium.listeners.jobs; // Adjusted package for now
...
public class FireballExplosion implements Listener {
    ...
    public FireballExplosion(PluginStart plugin) {
        this.plugin = plugin;
        plugin.getServer().getPluginManager().registerEvents(this, plugin);
        Text.sendDebugLog(INFO, "FireballExplosionListener enabled for fireballs with power > " + MIN_YIELD_FOR_REGEN);
    }
    ...
    public void onFireballExplode(EntityExplodeEvent event) {
        ...
        if (tasksCreatedForThisExplosion > 0) {
            ...
            Text.sendDebugLog(INFO, String.format("Fireball explosion (ID: %s) at %s (Power: %.1f) destroyed %d blocks. Scheduling regeneration.",
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if (remaining != null && remaining == 0) {
                            ...
                            if (details != null) {
                                Text.sendDebugLog(INFO, "Regeneration complete for fireball explosion " + task.explosionId.toString().substring(0,8) + ". Removing fire.");
    ...
    private void removeFireWithWorldEdit(ExplosionDetails details) {
        ...
        if (worldEditPlugin == null || !worldEditPlugin.isEnabled()) {
            Text.sendDebugLog(WARN, "WorldEdit not found or not enabled. Cannot remove fire for explosion at " + formatLocation(details.center));
            return;
        }
        ...
        Text.sendDebugLog(INFO, String.format("Attempting to remove fire in region (%d,%d,%d) to (%d,%d,%d) in world '%s' for explosion at %s",
        ...
        Text.sendDebugLog(INFO, "WorldEdit fire removal commands dispatched for explosion at " + formatLocation(details.center));
    }
