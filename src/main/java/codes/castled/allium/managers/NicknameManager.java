...
public class NicknameManager {
    ...
    public boolean setNickname(Player player, String nickname) {
        ...
        try {
            ...
            // Sync to EssentialsX if enabled
            syncNicknameToEssentials(player, nickname);
    ...
    public void resetNickname(Player player) {
        ...
        // Sync reset to EssentialsX if enabled (null/empty clears nickname)
        syncNicknameToEssentials(player, null);
        
        player.sendMessage(PrismaticAPI.colorize(player, "&aYour nickname has been reset to " + playerName));
    }

    /**
     * Syncs the nickname to EssentialsX storage using reflection.
     * This ensures /whois and other Essentials commands show the correct nickname.
     * @param player The player to sync
     * @param nickname The nickname to set (null to clear)
     */
    private void syncNicknameToEssentials(Player player, String nickname) {
        if (!plugin.getConfig().getBoolean("nickname.sync-to-essentials", true)) {
            plugin.getLogger().info("[Nick] sync-to-essentials is disabled in config");
            return;
        }
        ...
        plugin.getLogger().info("[Nick] Found Essentials plugin: " + essentials.getName());
        ...
        try {
            ...
            if (user == null) {
                plugin.getLogger().warning("[Nick] Essentials returned null User for " + player.getName());
                return;
            }
            
            plugin.getLogger().info("[Nick] Got Essentials User for " + player.getName() + ", calling setNickname...");
            
            // Call setNickname on the User object
            java.lang.reflect.Method setNickMethod = user.getClass().getMethod("setNickname", String.class);
            setNickMethod.invoke(user, nickname);
            
            // Also call setDisplayNick to update the display
            try {
                java.lang.reflect.Method setDisplayMethod = user.getClass().getMethod("setDisplayNick", String.class);
                setDisplayMethod.invoke(user, nickname);
                plugin.getLogger().info("[Nick] Also called setDisplayNick");
            } catch (NoSuchMethodException ignored) {
                // setDisplayNick might not exist in all versions
            }
            
            plugin.getLogger().info("[Nick] Successfully synced nickname to Essentials for " + player.getName() + ": " + (nickname != null ? nickname : "(cleared)"));
        } catch (NoSuchMethodException e) {
            plugin.getLogger().severe("[Nick] Essentials method not found: " + e.getMessage());
        } catch (Exception e) {
            plugin.getLogger().severe("[Nick] Error syncing to Essentials: " + e.getClass().getName() + ": " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void restoreDisplayNameFromStored(Player player, String storedNick) {
        ...
        try {
            ...
            // Sync to Essentials on join restore (ensures /whois is correct after restart)
            syncNicknameToEssentials(player, storedNick);
        } catch (Exception e) {
    ...
    private void applyDisplayIdentity(Player player, Component displayComponent) {
        // Always apply display name - Allium takes precedence over Essentials
        // The delay in ConnectionManager ensures this runs AFTER Essentials sets their display name
