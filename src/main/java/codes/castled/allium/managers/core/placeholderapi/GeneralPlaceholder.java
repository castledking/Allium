...
public class GeneralPlaceholder extends PlaceholderExpansion {
    ...
    /**
     * Get the player's fly status: yes (permanent fly on), no (fly off),
     * temporary (tfly on and flying), temporary-paused (tfly on but not flying).
     */
    private String getPlayerFlyStatus(Player player) {
        ...
        if (mgr != null && mgr.isTFlyEnabled(player.getUniqueId())) {
            return player.isFlying() ? "temporary" : "temporary-paused";
        }
    ...
    private String getPlayerGodStatus(OfflinePlayer player) {
        ...
        if (player.isOnline()) {
            ...
            try {
                ...
                Text.sendDebugLog(
                    WARN,
                    "Failed to retrieve god mode status for player " +
                        player.getName() +
                        ": " +
                        e.getMessage()
                );
    ...
    private String getReplyTarget(Player player) {
        try {
            ...
            codes.castled.allium.commands.Msg msgCommand =
                plugin.getMsgCommand();
            if (msgCommand == null) {
                return "";
            }

            // Use reflection to access the lastMessageSender map
            java.lang.reflect.Field lastMessageSenderField = msgCommand
                .getClass()
                .getDeclaredField("lastMessageSender");
            lastMessageSenderField.setAccessible(true);

            @SuppressWarnings("unchecked")
            Map<UUID, UUID> lastMessageSender = (Map<
                UUID,
                UUID
            >) lastMessageSenderField.get(msgCommand);
            ...
            Text.sendDebugLog(
                WARN,
                "Failed to get reply target for player " +
                    player.getName() +
                    ": " +
                    e.getMessage()
            );
    ...
    private String getUnreadMailCount(OfflinePlayer player) {
        ...
        try {
            ...
            Text.sendDebugLog(
                WARN,
                "Error getting unread mail count for " +
                    (player.getName() != null
                        ? player.getName()
                        : player.getUniqueId()),
                e
    ...
    private String getUnclaimedGiftsCount(OfflinePlayer player) {
        ...
        try {
            ...
            Text.sendDebugLog(
                WARN,
                "Error getting unclaimed gifts count for " +
                    (player.getName() != null
                        ? player.getName()
                        : player.getUniqueId()),
                e
    ...
    private String getPlayerDisplayName(Player player) {
        ...
        try {
            ...
            Text.sendDebugLog(
                WARN,
                "Error getting display name for " +
                    player.getName() +
                    ": " +
                    e.getMessage()
            );
