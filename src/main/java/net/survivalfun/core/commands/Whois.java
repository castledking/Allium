    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (targetOnlinePlayer != null) {
            ...
            try {
                ...
                Text.sendDebugLog(WARN, "Failed to get player data for '" + targetName + "': " + e.getMessage());
                Text.sendErrorMessage(sender, "player-not-found", lang, "name", targetName);
                return true;
            }
        }
        ...
        // Send header with player name
        lang.sendMessage(sender, "whois.header", "player_name", effectiveTarget.getName());

        // Create and send clickable UUID with copy functionality
        String uuid = effectiveTarget.getUniqueId().toString();
        if (sender instanceof Player) {
            Component uuidComponent = Text.colorize(lang.getRaw("whois.uuid"))
                .replaceText(builder -> builder.match("\\{uuid\\}").replacement(uuid))
                .clickEvent(ClickEvent.copyToClipboard(uuid))
                .hoverEvent(HoverEvent.showText(Component.text("Click to copy UUID to clipboard")));
            ((Player) sender).sendMessage(uuidComponent);
        } else {
            sender.sendMessage(Text.colorize(lang.getRaw("whois.uuid").replace("{uuid}", uuid)));
        }
        ...
        if (rawPrefix == null || rawPrefix.isEmpty() || rawPrefix.equals("N/A")) {
            prefixComponent = Component.text("N/A");
        } else {
            // Process gradient and color codes in the prefix
            prefixComponent = Text.colorize(rawPrefix);
        }
        
        String prefixTemplate = lang.getRaw("whois.prefix");
        // Create the final message component
        Component message = Text.colorize(prefixTemplate.replace("{prefix}", ""));
        // Add the prefix component with its formatting
        message = message.append(prefixComponent);
        ...
        if (targetOnlinePlayer != null) {
            ...
            try {
                ...
                Text.sendDebugLog(WARN, "Failed to get gamemode for " + targetName, e);
            }
        }
        String gamemodeDisplay;
        if (gameMode != null) {
            String gamemode = gameMode.name().toLowerCase();
            gamemodeDisplay = gamemode.substring(0, 1).toUpperCase() + gamemode.substring(1);
        } else {
            gamemodeDisplay = "Unknown";
        }
        sender.sendMessage(Text.colorize(lang.get("whois.gamemode").replace("{gamemode}", gamemodeDisplay)));
        ...
        if (targetOnlinePlayer != null) {
            ...
            if (lastLogoutLocation != null) {
                String worldPosMsgTemplate = lang.getRaw("whois.world-position");
                if (worldPosMsgTemplate.equals("whois.world-position")) {
                ...
                Map<String, Object> spawnLocationData = plugin.getDatabase().getSpawnLocation();
                String unknownLocationTemplate = "&eLocation: &f" + spawnLocationData.get("world") + " &7(Spawn)";
