...
public class Explode implements CommandExecutor {
    ...
    public void reloadConfig() {
        ...
        Text.sendDebugLog(INFO, "Reloaded explosion command configuration: min=" +
    ...
    private void createTNTSphere(Player player, int power) {
        ...
        // Log the attempt
        Text.sendDebugLog(INFO, "Creating TNT sphere with radius " + radius + " at " +
        ...
        if (worldEditPlugin != null && worldEditPlugin.isEnabled()) {
            try {
                ...
                Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    ...
                    Text.sendDebugLog(INFO, "Primed " + count + " TNT blocks");
                ...
                Text.sendDebugLog(ERROR, "Error creating TNT sphere with WorldEdit: " + e.getMessage());
                e.printStackTrace();

            }
        } else {
            Text.sendDebugLog(INFO, "WorldEdit not available, using manual TNT sphere creation");
        }
    }
    /**
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (sender instanceof Player playerSender) {
            ...
            if (targetPlayer.equals(playerSender)) {
                lang.sendMessage(sender, lang.get("explode.self").replace("{power}", String.valueOf(explosionPower)));
            } else {
                String otherMessage = lang.get("explode.other")
                        .replace("{name}", targetPlayer.getName())
                        .replace("{power}", String.valueOf(explosionPower));
            ...
            // Other sender types
            lang.sendMessage(sender, lang.get("not-a-player"));
            return true;
        }
    ...
    private void createFireball(Player player, int power) {
        ...
        // Log the attempt
        Text.sendDebugLog(INFO, "Creating fireball with power " + effectivePower + " at " +
                x + ", " + y + ", " + z);

        // Create the fireball directly via API (Folia-safe)
        try {
            ...
            Text.sendDebugLog(INFO, "Created fireball using direct API");
        } catch (Exception e) {
            Text.sendDebugLog(ERROR, "Failed to create fireball using direct API: " + e.getMessage());
        }
    ...
    private void applyFireballEffects(Player player, int power, double x, double y, double z) {
        ...
        if (power >= 16) {
            ...
            player.getWorld().strikeLightningEffect(new Location(player.getWorld(), x, y - 15, z));
        }
        ...
        if (power >= 76) {
            ...
            player.getWorld().playSound(player.getLocation(),
                    org.bukkit.Sound.ENTITY_WITHER_SPAWN, 0.5f, 0.8f);
