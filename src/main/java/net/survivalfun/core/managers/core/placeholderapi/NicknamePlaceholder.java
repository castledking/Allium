    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        ...
        if (params.equalsIgnoreCase("nickname_raw")) {
            try {
                ...
                return (stored != null && !stored.isEmpty()) ? stored : playerName;
            } catch (Throwable t) {
                return playerName;
            }
        }
        ...
        if (params.equalsIgnoreCase("nickname")) {
            try {
                ...
                // PrismaticAPI may return only color codes - tab plugins show blank. Use raw if no visible text.
                String stripped = stripColorCodes(result);
                if (stripped.isEmpty()) return nicknameToUse;
