...
public class CreeperExplosion implements Listener {
    ...
    public CreeperExplosion(PluginStart plugin) {
        this.plugin = plugin;
        ...
        if (instance != null) {
            ...
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, "[Allium] Unregistered previous CreeperExplosionListener instance");
            }
        }
    ...
    public void reloadConfig() {
        ...
        if (regenSpeed == 0) {
            Text.sendDebugLog(INFO, "Creeper explosion block regeneration is disabled");
        } else {
            Text.sendDebugLog(INFO, "Creeper explosion regeneration speed set to: " + regenSpeed);
        }
    }
    ...
    public void onCreeperExplode(EntityExplodeEvent event) {
        ...
        if (isProcessingLayers) {
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(WARN, String.format("[Allium] WARNING: New explosion while already processing layers! Queue size: %d", layerQueue.size()));
            }
        ...
        // Log the explosion
        if (plugin.getConfig().getBoolean("debug-mode")) {
            Text.sendDebugLog(INFO, String.format("Creeper explosion at %s destroyed %d blocks (minY=%d)",
        ...
        for (Map.Entry<Integer, List<Block>> entry : blocksByLayer.entrySet()) {
            ...
            if (skippedCount > 0) {
                if (plugin.getConfig().getBoolean("debug-mode")) {
                    Text.sendDebugLog(INFO, String.format("[Allium] Layer Y=%d: Skipped %d blocks already being regenerated", yLevel, skippedCount));
                }
            }
        }
        ...
        synchronized (layerQueue) {
            ...
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, String.format("[Allium] Adding layers to queue: %s", sortedYLevels.toString()));
            }
            ...
            for (int yLevel : sortedYLevels) {
                ...
                if (plugin.getConfig().getBoolean("debug-mode")) {
                    Text.sendDebugLog(INFO, String.format("[Allium] Added layer Y=%d to queue (queue size: %d)", yLevel, layerQueue.size()));
                }
            }
        }
        ...
        if (!isProcessingLayers) {
            Bukkit.getGlobalRegionScheduler().runDelayed(plugin, task -> {
                processNextLayer();
            }, 100); // 5 second delay before regeneration starts
        }
    }
    ...
    private void teleportPlayerToSafety(Player player) {
        ...
        if (safeLoc != null) {
            // Add debug logging
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, String.format("[Allium] Teleporting player %s from %s to %s for regeneration safety", 
                    player.getName(), 
            ...
            // Execute on the player's region thread
            Bukkit.getRegionScheduler().execute(plugin, safeLoc, () -> {
            ...
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, String.format("[Allium] No regen queue found, pushing player %s up 5 blocks to %s", 
                    player.getName(), 
    ...
    private void startHeartParticleEffect(Player player) {
        ...
        Bukkit.getRegionScheduler().runAtFixedRate(plugin, player.getLocation(), (ScheduledTask task) -> {
            ...
            if (ticksElapsed[0] % 4 == 0) {
                ...
                double angle = (ticksElapsed[0] * 0.2) % (2 * Math.PI); // Rotating pattern
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    double y = playerLoc.getY() + 1.5 + Math.sin(ticksElapsed[0] * 0.1 + i) * 0.3; // Gentle bobbing
            ...
            ticksElapsed[0]++;
        }, 0L, 1L); // Run every tick for smooth animation
    }
    ...
    private Location findTopOfRegenerationQueue(Location playerLocation) {
        ...
        for (Map.Entry<String, Integer> entry : highestYByColumn.entrySet()) {
            ...
            Location topBlock = new Location(world, x, y, z);
        ...
        if (!highestYByColumn.isEmpty()) {
            // Sort by Y level (highest first)
            List<Map.Entry<String, Integer>> sortedEntries = new ArrayList<>(highestYByColumn.entrySet());
            sortedEntries.sort((a, b) -> Integer.compare(b.getValue(), a.getValue()));
            Map.Entry<String, Integer> highest = sortedEntries.get(0);
            String[] coords = highest.getKey().split(",");
            int x = Integer.parseInt(coords[0]);
            int z = Integer.parseInt(coords[1]);
            int y = highest.getValue();
    ...
    private void processNextLayer() {
        synchronized (layerQueue) {
            if (layerQueue.isEmpty()) {
                if (plugin.getConfig().getBoolean("debug-mode")) {
                    Text.sendDebugLog(INFO, "[Allium] Layer queue is empty, stopping regeneration processing");
                }
            ...
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, String.format("[Allium] Processing layer Y=%d (queue remaining: %d)", currentLayer, layerQueue.size()));
            }
            ...
            if (currentLayerTasks == null || currentLayerTasks.isEmpty()) {
                if (plugin.getConfig().getBoolean("debug-mode")) {
                    Text.sendDebugLog(WARN, String.format("[Allium] No tasks found for layer Y=%d, skipping to next layer", currentLayer));
                }
            ...
            if (plugin.getConfig().getBoolean("debug-mode")) {
                Text.sendDebugLog(INFO, String.format("[Allium] Found %d tasks for layer Y=%d", currentLayerTasks.size(), currentLayer));
            }
                ...
                Bukkit.getRegionScheduler().runDelayed(plugin, task.location, (ScheduledTask scheduled) -> {
                    ...
                    if (isRestorableCurrentBlock(task.location.getBlock().getType())) {
                        ...
                        if (isOreMaterial(task.state.getType())) {
                            if (plugin.getConfig().getBoolean("debug-mode")) {
                                Text.sendDebugLog(INFO, String.format("[Allium] Skipping ore regeneration: %s at %s", 
                                    task.state.getType().name(), 
                        ...
                        if (state instanceof Container && isContainerMaterial(state.getType())) {
                            ...
                            if (placedBlock.getState() instanceof Container) {
                                ...
                                if (plugin.getConfig().getBoolean("debug-mode")) {
                                    Text.sendDebugLog(INFO, String.format("[Allium] Cleared inventory of regenerated container: %s at %s to prevent duping", 
                                        state.getType().name(), 
                                        String.format("%.0f,%.0f,%.0f", task.location.getX(), task.location.getY(), task.location.getZ())));
                                }
                            }
                    ...
                    if (remaining == 0) {
                        if (plugin.getConfig().getBoolean("debug-mode")) {
                            Text.sendDebugLog(INFO, String.format("[Allium] Layer Y=%d completed (%d/%d tasks), moving to next layer", 
    ...
    private void playRegenEffect(Location location) {
        try {
            ...
            Text.sendDebugLog(WARN, "Error playing regeneration effect: " + e.getMessage());
        }
    }
    
    private void startCleanupTask() {
        Bukkit.getGlobalRegionScheduler().runAtFixedRate(plugin, (ScheduledTask task) -> {
