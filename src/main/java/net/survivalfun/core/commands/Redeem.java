...
public class Redeem implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("allium.redeem")) {
            Map<String, String> noPermPlaceholders = new HashMap<>();
            noPermPlaceholders.put("cmd", label);
        ...
        for (String entry : new ArrayList<>(playersToRedeem)) { // Iterate over a copy to allow modification
            ...
            if (parts.length == 2) {
                try {
                    ...
                    Text.sendDebugLog(WARN, "Invalid UUID format in players-to-redeem list: " + parts[0] + " in entry '" + entry + "'");
                }
            } else {
                Text.sendDebugLog(WARN, "Invalid format in players-to-redeem list: '" + entry + "'. Expected UUID:Rank.");
            }
        }
        ...
        if (rankToRedeem != null) {
            ...
            if (commandsToRun.isEmpty()) {
                ...
                Text.sendDebugLog(WARN, "Redeem rank '" + rankToRedeem + "' is misconfigured (no commands found in 'redeem-settings.commands-to-run').");
                return true;
            }
