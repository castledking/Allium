...
public class PartyManager {
    ...
    // Per-player staff locator mode: player -> enabled
    // When enabled, player sees all players on locator bar regardless of distance
    private final Map<UUID, Boolean> staffLocatorMode = new ConcurrentHashMap<>();
    ...
    private boolean shouldBeVisibleToViewer(Player target, Player viewer) {
        ...
        // Players with allium.party.staff permission can see everyone regardless of distance
        if (viewer.hasPermission("allium.party.staff")) {
            return true;
        }
    ...
    private void handleNonPartyVisibility(Player viewer, Player target) {
        ...
        // Players with staff locator mode enabled can see everyone regardless of distance
        if (isStaffLocatorModeEnabled(viewer)) {
            showPlayerConsideringSpectator(viewer, target);
            return;
        }
    ...
    /**
     * Toggle per-player staff locator mode between seeing all players and radius-based visibility.
     * @param player The player whose mode to toggle
     * @return true if now in staff mode, false if in normal mode
     */
    public boolean toggleStaffLocatorMode(Player player) {
        UUID playerId = player.getUniqueId();
        boolean currentMode = staffLocatorMode.getOrDefault(playerId, player.hasPermission("allium.party.staff"));
        boolean newMode = !currentMode;
        staffLocatorMode.put(playerId, newMode);
        
        // Refresh visibility for this player
    ...
    /**
     * Check if a player has staff locator mode enabled (sees all players).
     * @param player The player to check
     * @return true if staff locator mode is enabled
     */
    public boolean isStaffLocatorModeEnabled(Player player) {
        UUID playerId = player.getUniqueId();
        return staffLocatorMode.getOrDefault(playerId, player.hasPermission("allium.party.staff"));
    }

    /**
     * Get the current staff locator mode status for a player.
     * @param player The player to check
     * @return true if in staff mode (sees all players), false if normal mode (radius-based)
     */
