...
public final class AlliumChannelManager implements Listener {
    ...
    private volatile String localChannelName = "local";
    private volatile int localChannelRadius = 100;
    ...
    public AlliumChannelManager(PluginStart plugin) {
        this.plugin = plugin;
        this.playerChannelsFile = new File(plugin.getDataFolder(), "player-channels.yml");
        loadPlayerChannels();
        reload();
    ...
    private void loadPlayerChannels() {
        ...
        try {
            ...
            plugin.getLogger().warning("[Channels] Failed to load player-channels.yml: " + e.getMessage());
            playerChannelsConfig = new YamlConfiguration();
        }
    }

    private void savePlayerChannels() {
        if (playerChannelsConfig == null) {
            playerChannelsConfig = new YamlConfiguration();
        }
        try {
            playerChannelsConfig.save(playerChannelsFile);
        } catch (Exception e) {
            plugin.getLogger().warning("[Channels] Failed to save player-channels.yml: " + e.getMessage());
        }
    }

    private String getSavedChannel(UUID playerId) {
        if (playerChannelsConfig == null) {
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] getSavedChannel: playerChannelsConfig is null for " + playerId);
            }
            return null;
        }
        String channel = playerChannelsConfig.getString("channels." + playerId.toString());
        if (plugin.isDebugMode()) {
            Text.sendDebugLog(INFO, "[Channels] getSavedChannel: " + playerId + " -> '" + channel + "'");
        }
        return channel;
    }

    private void setSavedChannel(UUID playerId, String channel) {
        if (playerChannelsConfig == null) {
            playerChannelsConfig = new YamlConfiguration();
        }
        playerChannelsConfig.set("channels." + playerId.toString(), channel);
        savePlayerChannels();
        if (plugin.isDebugMode()) {
            Text.sendDebugLog(INFO, "[Channels] setSavedChannel: saved " + playerId + " -> '" + channel + "' to " + playerChannelsFile.getAbsolutePath());
        }
    }

    private void clearSavedChannel(UUID playerId) {
        if (playerChannelsConfig == null) return;
        playerChannelsConfig.set("channels." + playerId.toString(), null);
        savePlayerChannels();
    }
    ...
    public void reload() {
        ...
        // Load local channel settings
        ConfigurationSection localSection = root != null ? root.getConfigurationSection("local-channel") : null;
        if (localSection != null) {
            localChannelName = "local";
            localChannelRadius = localSection.getInt("radius", 100);
            String localFormat = localSection.getString("format", "&8[&eLocal&8] <prefix> &a%allium_nickname%&f: &f<message>");
            List<String> localAliases = localSection.getStringList("aliases");
            if (localAliases.isEmpty()) {
                localAliases = List.of("local", "l");
            }
            boolean excludeFromDiscord = localSection.getBoolean("exclude-from-discord", true);

            // Create local channel definition
            ChannelDefinition localDef = new ChannelDefinition(
                    localChannelName,
                    "Local",
                    localAliases,
                    localFormat,
                    excludeFromDiscord ? "__LOCAL_NO_DISCORD__" : "", // Special marker to skip Discord
                    "", // discordInFormat
                    "", // discordOutFormat
                    false, // webhookDelivery
                    true, // isLocal
                    localChannelRadius
            );
            channels.put(localChannelName, localDef);
            aliasToChannel.put(localChannelName, localChannelName);
            for (String alias : localAliases) {
                aliasToChannel.putIfAbsent(normalize(alias), localChannelName);
            }

            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] Loaded local channel: radius=" + localChannelRadius + " aliases=" + localAliases + " no-discord=" + excludeFromDiscord);
            }
        }
    ...
    public void applyAutojoin(Player player, boolean skipWriteChannel) {
        ...
        for (ChannelDefinition channel : channels.values()) {
            ...
            // Only autojoin write channel if no saved preference exists
            if (!skipWriteChannel && player.hasPermission("allium.channel.autojoin." + channelName + ".write")) {
                if (chosenWrite == null) {
        ...
        // Default to default channel only if no saved preference and no autojoin write found
        if (!skipWriteChannel && chosenWrite == null && channels.containsKey(defaultChannelName)) {
    ...
    public boolean canRead(Player player, String channelName) {
        ...
        // Local channel is readable by all players by default (radius will filter)
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        plugin.getLogger().info("[Channels] onJoin fired for " + player.getName() + " enabled=" + enabled);

        if (!enabled) {
            plugin.getLogger().info("[Channels] onJoin skipping - channels not enabled");
            return;
        }

        if (plugin.isDebugMode()) {
            Text.sendDebugLog(INFO, "[Channels] onJoin START for " + player.getName() + " uuid=" + player.getUniqueId());
            Text.sendDebugLog(INFO, "[Channels] playerChannelsFile exists=" + playerChannelsFile.exists() + " size=" + (playerChannelsFile.exists() ? playerChannelsFile.length() : 0));
        }

        // Load saved channel preference
        String savedChannel = getSavedChannel(player.getUniqueId());
        boolean hasSavedChannel = savedChannel != null && !savedChannel.isEmpty();

        if (plugin.isDebugMode()) {
            Text.sendDebugLog(INFO, "[Channels] onJoin hasSavedChannel=" + hasSavedChannel + " savedChannel='" + savedChannel + "'");
        }

        // Apply autojoin (sets up read channels, and write channel only if no saved preference)
        applyAutojoin(player, hasSavedChannel);

        // Restore saved write channel AFTER autojoin (overrides autojoin write channel)
        if (hasSavedChannel) {
            String resolved = resolveChannel(savedChannel);
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] onJoin resolving '" + savedChannel + "' -> '" + resolved + "'");
            }
            if (resolved == null) {
                if (plugin.isDebugMode()) {
                    Text.sendDebugLog(INFO, "[Channels] Could not restore channel for " + player.getName() + " - resolved is null for '" + savedChannel + "'");
                }
            } else if (!canWrite(player, resolved)) {
                if (plugin.isDebugMode()) {
                    Text.sendDebugLog(INFO, "[Channels] Could not restore channel for " + player.getName() + " - no write permission for " + resolved);
                }
            } else {
                writeChannels.put(player.getUniqueId(), resolved);
                addReadChannel(player.getUniqueId(), resolved);
                if (plugin.isDebugMode()) {
                    Text.sendDebugLog(INFO, "[Channels] Restored " + player.getName() + " write channel to " + resolved + " (overriding autojoin)");
                }
            }
        } else {
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] No saved channel for " + player.getName() + " (savedChannel=" + savedChannel + ")");
            }
        }

        // Log final state
        if (plugin.isDebugMode()) {
            String finalWrite = writeChannels.get(player.getUniqueId());
            Text.sendDebugLog(INFO, "[Channels] onJoin END for " + player.getName() + " finalWriteChannel='" + finalWrite + "'");
        }
    }
    ...
    public void onQuit(PlayerQuitEvent event) {
        ...
        // Save current write channel preference (always save, not just non-default)
        String currentWrite = writeChannels.get(playerId);
        if (currentWrite != null) {
            setSavedChannel(playerId, currentWrite);
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] Saved " + event.getPlayer().getName() + " channel '" + currentWrite + "' on quit");
            }
        } else {
            clearSavedChannel(playerId);
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] Cleared saved channel for " + event.getPlayer().getName() + " (no write channel on quit)");
            }
        }
    ...
    public void onPlayerChatEarly(AsyncChatEvent event) {
        ...
        boolean isLocalChannel = targetChannel.equals(localChannelName);
    ...
    public void onPlayerChat(AsyncChatEvent event) {
        ...
        boolean isLocalChannelFinal = targetChannel.equals(localChannelName);
        recentOutboundChats.put(player.getUniqueId(), new RecentOutboundChat(
                normalizeMessageForComparison(message),
                System.currentTimeMillis(),
                targetChannel.equals(staffChannelName),
        ...
        for (Player recipient : Bukkit.getOnlinePlayers()) {
            ...
            if (isLocalChannel && radius > 0) {
                if (!sender.getWorld().equals(recipient.getWorld())) {
                    continue; // Different worlds - can't hear local chat
                }
    ...
    private boolean isReading(UUID playerId, String channel) {
        // Local channel is readable by all players (radius filters recipients)
        if (channel.equals(localChannelName)) {
            return true;
        }
    ...
    private void relayToDiscord(Player sender, ChannelDefinition channel, String plainMessage) {
        ...
        // Skip Discord relay for local channels (they have special marker "__LOCAL_NO_DISCORD__")
        if (channel.discordChannelId().equals("__LOCAL_NO_DISCORD__")) {
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] Discord relay skipped for local channel: " + channel.name());
            }
            return;
        }
    ...
    private boolean shouldSuppressOutboundRelay(UUID playerId, org.bukkit.event.Event triggeringEvent, String message) {
        ...
        if (plugin.isDebugMode()) {
            Text.sendDebugLog(INFO, "[Channels] shouldSuppress check: isShortcut=" + recent.isShortcut() + " isLocal=" + recent.isLocal() + " message='" + message + "' recent.message='" + recent.message() + "'");
        }

        // Suppress if this was a local channel message (never goes to Discord)
        if (recent.isLocal()) {
            return true;
        }
    ...
    private void relayDiscordMessageToChannel(ChannelDefinition channel, String authorName, String effectiveName,
        ...
        String format;
        boolean useAlliumFormat = false;
        // Only use channel-specific format if it's a properly configured channel (not a fallback)
        // This ensures unmapped Discord channels use DiscordSRV's native format
        if (!channel.name().equals(defaultChannelName) && channel.discordInFormat() != null && !channel.discordInFormat().isBlank()) {
            // Use channel-specific discord-in-format with Allium placeholder parsing
            format = channel.discordInFormat();
            useAlliumFormat = true;
            if (plugin.isDebugMode()) {
                Text.sendDebugLog(INFO, "[Channels] Using custom discord-in-format (Allium format): " + format);
            }
        } else {
            // Use DiscordSRV's configured format (for default channel and unmapped channels)
            format = getDiscordSrvMessageFormat();
            useAlliumFormat = false;
            if (plugin.isDebugMode()) {
    ...
    private String formatDiscordSrvMessage(String format, String username, String effectiveName,
        ...
        String parsedReplySnippet = parseDiscordEmojis(replySnippet);

        String result;

        if (useAlliumFormat) {
            // Use Allium-style placeholders for Allium custom formats
            result = format
                    .replace("{discord_username}", parsedEffectiveName != null && !parsedEffectiveName.isBlank() ? parsedEffectiveName : parsedUsername)
                    .replace("{player}", parsedUsername)
                    .replace("{message}", MINI_MESSAGE.escapeTags(parsedMessage))
                    .replace("{channel}", parsedChannelName != null ? parsedChannelName : "")
                    .replace("{role_alias}", parsedRoleAlias != null && !parsedRoleAlias.isBlank() ? parsedRoleAlias : "")
                    .replace("{role_color}", parsedRoleColor != null && !parsedRoleColor.isBlank() ? parsedRoleColor : "");
        } else {
            // Use DiscordSRV-style placeholders
            result = format
                    .replace("%name%", parsedUsername)
                    .replace("%displayname%", parsedEffectiveName != null && !parsedEffectiveName.isBlank() ? parsedEffectiveName : parsedUsername)
                    .replace("%username%", parsedUsername)
                    .replace("%message%", parsedMessage)
                    .replace("%channelname%", parsedChannelName != null ? parsedChannelName : "")
                    .replace("%toprolealias%", parsedRoleAlias != null && !parsedRoleAlias.isBlank() ? parsedRoleAlias : "")
                    .replace("%toprolecolor%", parsedRoleColor != null && !parsedRoleColor.isBlank() ? parsedRoleColor : "");

            // %reply% placeholder - if it's a reply, show the reply snippet, otherwise empty
            if (isReply && parsedReplySnippet != null && !parsedReplySnippet.isBlank()) {
                result = result.replace("%reply%", parsedReplySnippet);
            } else {
    ...
    public record ChannelDefinition(
        ...
        // Convenience constructor for regular channels (non-local)
        public ChannelDefinition(
            ...
            this(name, displayName, aliases, format, discordChannelId, discordInFormat, discordOutFormat, webhookDelivery, false, 0);
        }
    }
