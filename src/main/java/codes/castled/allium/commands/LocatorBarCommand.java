...
public class LocatorBarCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "help":
                if (!player.hasPermission("allium.party.staff") && !player.hasPermission("allium.locatorbar.help")) {
                    player.sendMessage(Text.colorize("&cYou don't have permission to view the help."));
                    return true;
                }
            ...
            default:
                player.sendMessage(Text.colorize("&cUnknown subcommand. Use /locatorbar help for help."));
                break;
        }
    ...
    private void handleToggle(Player player) {
        // Check if player has permission for staff locator mode
        if (!player.hasPermission("allium.party.staff")) {
            player.sendMessage(Text.colorize("&cYou don't have permission to use staff locator mode."));
            return;
        }

        boolean newMode = partyManager.toggleStaffLocatorMode(player);
        if (newMode) {
            player.sendMessage(Text.colorize("&aStaff locator mode enabled: You can now see all players on the locator bar."));
        } else {
            player.sendMessage(Text.colorize("&eStaff locator mode disabled: You now only see players within the configured radius."));
        }
    }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            for (String sub : new String[]{"toggle", "help", "show", "hide", "reset"}) {
